class RefConResults
  ...
  def self.data(params)
    if cid = params[:contest_id]
      ...
      precinct_results = results.group('candidate_id, precinct_id').select('candidate_id, precinct_id, sum(votes) v').order('v DESC')
      { candidates: Candidate.where(id: cids).select("id, name, party").map { |c| { id: c.id, name: c.name, party: c.party } },
        ...
        precinctResults: precinct_results.group_by(&:precinct_id).map { |precinct_id, results| [ precinct_id, results[0, 2].map { |r| { cid: r.candidate_id, votes: r.v } } ] }
      }
    else
      {}
    end
